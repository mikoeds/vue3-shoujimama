...
import { deletActivityInfo, getActivityInfo } from '../../utiles/api/selectSpecial';
import type { activityInfoType } from '../../types/selectSpecial';
import { getDeliverData } from '../../utiles/api/freighTemplate';
import type { deliverInfoDataType, deliverInfoParamType } from '../../types/freighTemplate';

let pageSize = ref(10)
let pageNumber = ref(1)
let total = ref(0)
let activityList = reactive<deliverInfoDataType[]>([])
// 获取数据参数
let getInfoData = reactive<deliverInfoParamType>({
    deliver_name: ""
})
...
const handleGetInfo = async () => {
    statusChange(true)
    await getDeliverData<Array<deliverInfoDataType>, deliverInfoParamType>(getInfoData).then(res => {
        activityList.length = 0;
        console.log(res);
        activityList.push(...res.result);
    }).finally(() => {
        statusChange(false);
    });
};
...
const handleDelete = (idArray: Array<number>) => {
    statusChange(true);
    deletActivityInfo({ idArray }).then(async (res) => {
        console.log(res);
        await handleGetInfo();
    }).catch().finally(() => {
        statusChange(false);
...
<template>
    <div>
        ...
        <div class="max">
            <el-input v-model="getInfoData.deliver_name" palceholder="请输入分类名称"></el-input>
        ...
        <div class="max">
            ...
            <el-button type="danger" :icon="Delete" @click="handleDeletChosen"
                :disabled="disableButton">批量删除</el-button>
        </div>

        <el-table border :data="activityList.slice((pageNumber - 1) * pageSize, pageNumber * pageSize)" ref="tableRef"
            v-loading="loading">
            <el-table-column type="index"></el-table-column>
            <el-table-column type="selection"></el-table-column>
            <el-table-column label="模板名称" prop="deliver_name"></el-table-column>
